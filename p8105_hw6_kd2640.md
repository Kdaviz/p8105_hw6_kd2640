...
birthweight %>% 
 ...
 fit = lm(bwt ~ bhead + blength + delwt + gaweeks + malform + smokes, data = birthwt)
 summary(fit)
```
...
```
    ...
    ## # … with 3,463 more rows, and 9 more variables: mheight <int>,
    ## #   momage <int>, mrace <fct>, parity <int>, ppbmi <dbl>, ppwt <int>,
    ## #   smoken <dbl>, wtgain <int>, smokes <fct>
...
cv_birthwt %>% 
  ...
  mutate(
         model = recode(
           model, 
           bwt_model1 = "Hypothesized Model", 
           bwt_model2 = "Small Model",
          bwt_model3 = "Model with Interactions"), 
...
<img src="p8105_hw6_kd2640_files/figure-markdown_github/unnamed-chunk-7-1.png" width="90%" />

When comparing the three models, the hypothesized model is the model I would choose because it has the smallest RMSE. The small model has the largest RMSEs and the interaction model has a small RMSE as well but not as small as the hypothesized model. The interaction model will e harder to interpret, so hypothesized model is a better choice to use.
